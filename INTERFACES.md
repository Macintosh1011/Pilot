...
- **Internal LLM worker queue:** `finalize.finalize` keeps the same public signature, but internally queues a local Codex worker job and schedules a deterministic safety fallback so cards never hang if the worker is offline. B1/B3 do not need to call the worker APIs.
